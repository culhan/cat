...
class Ion_auth_model extends CI_Model
{
	...
	**/
	 ...
	 * @author aditya menon
	*/
	public function createData( array $data, $tables )
	{
		// insert the new group
		$this->db->insert($this->tables[$tables], $data);
		$group_id = $this->db->insert_id();
	 ...
	 * @author aditya menon
	*/
	public function deleteData( $id, $tables )
	{
		// insert the new group
		$this->db->delete($this->tables[$tables], array('id' => $id));
		// report success
		$this->set_message('deletion_successful');
	}
	 ...
	 * @author aditya menon
	*/
	public function updateData( $id, array $data, $tables )
	{
		if( is_array($id) )
		{
			$result = $this->db->update($this->tables[$tables], $data, $id);
		}
		else
		{
			// update the new group
			$result = $this->db->update($this->tables[$tables], $data, array('id' => $id));
		}
	 ...
	 * @author Ben Edmunds
	 **/
	public function getData( $id = NULL, $tables )
	{

		if (isset($id))
		{
			$this->where($this->tables[$tables].'.id', $id);
		}

		$this->limit(1);
		$this->order_by('id', 'desc');

		return $this->allData( $tables );
	}
	 ...
	 * @author Ben Edmunds
	 **/
	public function allData( $tables )
	{
		...
		$this->response = $this->db->get($this->tables[$tables]);

		return $this;
	}

	public function leftJoin( $table, $tabelColumn, $tableLeft, $tableLeftColumn )
	{
		$this->db->join( $tableLeft, $tableLeft . '.' . $tableLeftColumn . ' = ' . $table . '.'. $tabelColumn, 'left');
